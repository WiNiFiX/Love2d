...
local function ScaleToPO2(xsize, ysize)
   if supported.npot then return xsize, ysize end
...
function CreateBloomEffect(xsize, ysize)     
    if not love.graphics.newShader then print('not love.graphics.newShader is the issue.') return end
    if not supported then print('not love.graphics.isSupported is the issue.') return end    
    if not supported.shaderderivatives then print('not love.graphics.isSupported("shader") is the issue.') return end
    if not supported.multicanvasformats then print('not love.graphics.isSupported("canvas") is the issue.') return end      
